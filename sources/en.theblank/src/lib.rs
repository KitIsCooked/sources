...
use aidoku::{
    ...
    Home, HomeLayout, Listing, ListingProvider, Manga, MangaPageResult, MangaStatus,
...
impl Source for Theblank {
    ...
    fn get_search_manga_list(
        ...
        let props: LibraryProps = parse_inertia(&html)
            .ok_or(AidokuError::Message(String::from("failed to parse library")))?;

        let has_next = props.series.meta.current_page < props.series.meta.last_page;
        let entries = props.series.data.into_iter().map(|s| Manga {
            key: s.link,
            title: s.title,
    ...
    fn get_manga_update(
        ...
        let html = fetch_html(&url)?;
        let props: SerieDetailProps = parse_inertia(&html)
            .ok_or(AidokuError::Message(String::from("failed to parse serie page")))?;

        let s = props.serie;
        let tags: Vec<String> = s.genres.into_iter().map(|g| g.name).collect();

        let chapters = if needs_chapters {
            Some(s.chapters.into_iter().map(|c| {
    ...
    fn get_page_list(&self, _manga: Manga, chapter: Chapter) -> Result<Vec<Page>> {
        ...
        let html = fetch_html(&url)?;
        let props: ChapterReaderProps = parse_inertia(&html)
            .ok_or(AidokuError::Message(String::from("failed to parse chapter page")))?;
        ...
        let pages = (1..=d.page_count).map(|i| Page {
